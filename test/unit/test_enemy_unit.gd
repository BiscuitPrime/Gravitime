...
func test_save():
	...
	enemy.save()
	assert_eq(enemy.timeposition,Vector2(0,0),"Should fail : l'enemy, placé en (45,56) effectue son save et donc timeposition=Vector2(45,56)")
...
func test_timeReset():
	...
	enemy.timeReset()
	assert_eq(enemy.position,Vector2(3,3), "Should fail : l'enemy, placé en (3,3) revient à sa position timeposition=(45,56) donc position=(45,56) != (3,3)")
...
func test_calculate_new_velocity():
	...
	assert_eq(new_velocity, Vector2(2,2), "Should fail : la vélocité initiale (2,2) est transformée par calculate_new_velocity(velocity)")
